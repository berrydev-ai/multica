...
| Jcode transport/server-pinning args and MCP | `pkg/agent/jcode.go` `jcodeBlockedArgs` / `Execute` | `custom_args` reach `jcode acp` argv, but subcommand tokens and `--socket` / `--resume` / `--remote-working-dir` are filtered: tasks are pinned to the Multica-scoped persistent server (`JCODE_RUNTIME_DIR`/`JCODE_SOCKET` env, injected last so nothing overrides them). Provider/model selectors (`-p`, `--model`, `--provider-profile`) pass through. `mcp_config` is never forwarded — jcode's ACP surface validates and drops `mcpServers`, so the MCP tab is hidden for jcode agents. |
