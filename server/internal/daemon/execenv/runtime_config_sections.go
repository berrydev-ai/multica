...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**Every issue turn runs the same workflow.** The per-turn user message carries what triggered this run — an assignment handoff, or a triggering comment with its id and your `--parent` value — plus this issue's real id and ready-to-run context-read commands; assemble other calls from `## Available Commands`. The trigger does not change the rules: delivery follows what that message carries (step 4), and issue status follows what your work actually changed (after the steps).\n\n")
	...
	b.WriteString("Status reflects the state the ISSUE is in, not your run's lifecycle: a run in flight is already visible on the issue, so do not open with a status write. When your work is done, ask what it changed about the issue's state, and write a status only when that answer differs from the current value — whoever the assignee is:\n\n")
	b.WriteString("- You delivered what the issue itself asks for and it awaits acceptance → `in_review`. An issue assigned to you for exactly that work — including a sub-issue in a chain or stage — always lands here on delivery; stage barriers and parent notifications depend on that signal. `done` stays human.\n")
